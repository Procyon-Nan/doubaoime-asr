...
from .schemas import ErrorResponse, LegacyRequest, StandardRequest, error, ok_text, ok_transcription
...
class ASRRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        if self.path == self.server.options.health_path:
            self._send_json(
                HTTPStatus.OK,
    ...
    def do_POST(self) -> None:
        if self.path == self.server.options.bridge_path:
            self._handle_legacy_transcribe()
            return
        if self.path == self.server.options.standard_path:
